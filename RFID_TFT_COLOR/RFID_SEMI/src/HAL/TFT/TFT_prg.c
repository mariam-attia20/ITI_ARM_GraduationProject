...
/* CS was previously not controlled at all here (fine when the TFT was the
 * only device on the bus - CS just gets tied to GND on the module). Now
 * that RC522 shares SPI1, the TFT needs its OWN software-controlled CS so
 * it only listens during its own transactions, otherwise it would also
 * pick up every byte meant for the RC522. Wire the TFT's CS pin to PA2
 * instead of GND. */
GPIOx_PinConfig_t TFT_CS_PIN={
		.Port = GPIO_PORTA,
		.Pin = GPIO_PIN2,
		.Mode = GPIO_MODE_OUTPUT,
		.Speed = GPIO_SPEED_HIGH,
...
static void CS_Low(void)  { MGPIO_vSetPinValue(TFT_CS_PIN.Port, TFT_CS_PIN.Pin, GPIO_LOW); }
static void CS_High(void) { MGPIO_vSetPinValue(TFT_CS_PIN.Port, TFT_CS_PIN.Pin, GPIO_HIGH); }
